...
from llm import modelInference
...
def get_pred(
    ...
    for json_obj in tqdm(data):
        ...
        if model_name == "llama-3-inst":
            extra_end_token_ids.append(searcher.tokenizer.encode("<|eot_id|>", add_special_tokens=False)[0])

        if model_name == "qwen":
            extra_end_token_ids.append(searcher.tokenizer.encode("<|im_end|>", add_special_tokens=False)[0])

        if dataset == "samsum":
            extra_end_token_ids.append(searcher.tokenizer.encode("\n", add_special_tokens=False)[-1])

        if dataset not in ["trec", "triviaqa", "samsum", "lsht", "lcc", "repobench-p"]: 
            # chat models are better off without build prompts on these tasks
            prompt = build_chat(searcher.tokenizer, prompt, model_name)
        ...
        tokenized_prompt = searcher.tokenizer(prompt, truncation=False, return_tensors="pt", add_special_tokens=add_special_tokens).input_ids[0]
            ...
            if truncation == "suffix":
                ...
                if length > max_length - max_gen:
                    if verbose:
                        print("over length")
                    init_token_num = 128
                    prompt = searcher.tokenizer.decode(tokenized_prompt[:init_token_num].tolist() + tokenized_prompt[- (max_length - max_gen - init_token_num):].tolist())
                    tokenized_prompt = searcher.tokenizer(prompt, truncation=False, return_tensors="pt", add_special_tokens=add_special_tokens).input_ids[0]
            else:
                raise NotImplementedError
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        if dataset in set([
            ...
            path = "data/infinite-bench"
            data = load_infinite_bench(path, dname)

        else:
            data = load_from_disk(
                f"data/longbench/{dataset}"
            )
